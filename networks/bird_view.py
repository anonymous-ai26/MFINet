...
class BEVNet(nn.Module):
    def __init__(self, base_block, context_layers, layers, use_att):
        ...
        self.MSDA0 = MSDA(dim=32, num_heads=8, kernel_size=3, dilation=[1, 2])
        self.MSDA1 = MSDA(dim=64, num_heads=8, kernel_size=3, dilation=[1, 2])
        self.MSDA2 = MSDA(dim=128, num_heads=8, kernel_size=3, dilation=[1, 2])
        ...
        self.up0 = AttMerge(context_layers[0], fusion_channels1 // 2, fusion_channels0 // 2, size=(600, 600))
    ...
    def forward(self, x):
        ...
        x_merge1_up = F.interpolate(x_merge1, size=x.size()[2:], mode='bilinear', align_corners=True)
        x_merge0_up = F.interpolate(x_merge0, size=x.size()[2:], mode='bilinear', align_corners=True)

        return x_merge, x_merge1_up, x_merge0_up
