    ...
    with torch.no_grad():
        for i, (pcds_xyzi, pcds_coord, pcds_sphere_coord, pcds_target, fname_pcd) in tqdm.tqdm(enumerate(val_loader)):
            with torch.cuda.amp.autocast():
                # pred_cls, bev_pred_cls, rv_pred_cls, bev_input_target, rv_input_traget = model.infer(pcds_xyzi.squeeze(0).cuda(), pcds_coord.squeeze(0).cuda(), pcds_sphere_coord.squeeze(0).cuda(), pcds_target.cuda())
                # pred_cls, bev_pred_cls, rv_pred_cls, bev_input_target, rv_input_traget = model.infer(pcds_xyzi.cuda(), pcds_coord.cuda(), pcds_sphere_coord.cuda(), pcds_target.cuda())
                # pred_cls, bev_pred_cls, rv_pred_cls, bev_input_target, rv_input_traget, bev_merge1_up_cls, bev_merge0_up_cls, rv_merge1_up_cls, rv_merge0_up_cls = model.infer(pcds_xyzi.cuda(), pcds_coord.cuda(), pcds_sphere_coord.cuda(), pcds_target.cuda())
                pred_cls, bev_pred_cls, rv_pred_cls, bev_input_target, rv_input_traget, bev_merge1_up_cls, bev_merge0_up_cls, rv_merge1_up_cls, rv_merge0_up_cls = model.infer(pcds_xyzi.squeeze(0).cuda(), pcds_coord.squeeze(0).cuda(), pcds_sphere_coord.squeeze(0).cuda(), pcds_target.cuda())
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', help='config file path', default='config/mfinet.py', type=str)
    parser.add_argument('--local_rank', type=int, default=0)

    parser.add_argument('--start_epoch', type=int, default=23)
    parser.add_argument('--end_epoch', type=int, default=23)
